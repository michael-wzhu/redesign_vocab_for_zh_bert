...
class SubwordTextEncoder(TextEncoder):
    ...
    def build_to_target_size(cls,
        ...
        def bisect(min_val, max_val, subtokenizer):
            """Bisection to find the right size."""
            present_count = (max_val + min_val) // 2
            # tf.logging.info("Trying min_count %d" % present_count)
            print("Trying min_count %d" % present_count)
            # subtokenizer = cls()
            ...
            if subtokenizer.vocab_size > target_size:
                other_subtokenizer = bisect(present_count + 1, max_val, subtokenizer)
            else:
                other_subtokenizer = bisect(min_val, present_count - 1, subtokenizer)
        ...
        return bisect(min_val, max_val, cls())
